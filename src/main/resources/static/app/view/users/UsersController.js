...
Ext.define("App.view.users.UsersController", {
	...
	uses: ["App.view.users.UsersWin"],
	
	//初始化
	init: function() {
		this.st = Ext.getCmp("usersGrid").getStore(); //通过Component获取Store
	},
	...
	//新增
	add: function() {
		var win = Ext.create("App.view.users.UsersWin");
		win.show();
	},
	...
	//保存
	save: function(btn) {
		var fr = this.lookupReference("usersForm").getForm();
		if(fr.isValid()) {
